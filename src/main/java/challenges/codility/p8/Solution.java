package challenges.codility.p8;
 ...
 * from 1 to X (that is, we want to find the earliest moment when all the positions from 1 to X are covered by leaves). You may assume that the speed of the current in the river is
...
public class Solution {
    public int solution(int X, int[] A) {
        Map<Integer, Integer> path = new HashMap<>(A.length);
        for (int i = 0; i < A.length; i++) {
            Integer t = path.get(A[i]);
            if (t == null || t > i)
                path.put(A[i], i);
            else
                path.put(A[i], t);
        }
        if (!path.containsKey(X)) return -1;
        Integer time = path.get(X);
        for (int i = 1; i <= X; i++)
        {
